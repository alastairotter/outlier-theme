...
First install the necessary packages using install.packages("packagename"). [See names in libraries below]
...
#### Save barchart as an SVG file for Figma
```{r barchartsvg}
ggsave(file="test.svg", plot=barChart, width=10, height=5) # set file name
```
...
```{r linesvg}
ggsave(file="line.svg", plot=lineChart, width=10, height=5)
```
